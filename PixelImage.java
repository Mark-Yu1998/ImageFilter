...
public class PixelImage
{
	...
	public Pixel[][] calculate(int[][] weight, Pixel[][] data,boolean needScaling){
		Pixel[][] newData_Out = new Pixel[data.length][data[0].length];
		int half =  weight.length / 2;
		for (int row = 0;row < data.length;row++) {
			for (int col = 0;col < data[row].length;col++) {
				if (row < half || col < half || row > data.length - half - 1 || col > data[row].length - half - 1) {
					...
					if (needScaling) {
						newData_Out[row][col] = new Pixel(red / 16,green / 16, blue / 16);
					}else {
						newData_Out[row][col] = new Pixel(Math.max(0, Math.min(255, red)),Math.max(0, Math.min(255, green)),Math.max(0, Math.min(255, blue)));
					}
